...
func _on_velocity_changed(new_velocity: Vector3, _acceleration: Vector3):
	velocity = new_velocity
	_tilt_y_component.movement_direction = new_velocity


func _on_player_move(direction: Vector2):
	_velocity_component.direction = Vector3(direction.x, direction.y, 0)
...
func _on_player_control_component_player_rolls():
	if _velocity_component.direction != Vector3.ZERO:
...
func _on_barrel_roll_ended(acceleration: Vector3):
